...
class RentalTextFileRepository(MemoryRepository):
    ...
    def __load(self):
        ...
        while len(line) > 0:
            ...
            start_date = datetime.strptime(tokens[3], "%Y-%m-%d")
            end_date = datetime.strptime(tokens[4].strip(), "%Y-%m-%d")

            # NOTE We assume that input files are correct
    ...
    def __save(self):
        ...
        for rental in self._data.values():
            # CSV - file
            fout.write(str(rental.id) + "," + str(rental.car.id) + "," + str(rental.client.id) + "," +
                       rental.start.strftime("%Y-%m-%d") + "," + rental.end.strftime("%Y-%m-%d") + "\r\n")
        fout.close()
